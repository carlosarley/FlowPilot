...
// Mobile feature pills — Lucide icons for cross-OS consistency
const MOBILE_PILLS = [
 ...
 * 1. Dot grid background (radial-gradient) replaces generic blobs — more editorial
 * 2. Gradient text on the H1 accent span (.text-gradient-teal) adds depth
 * 3. MockupUI floats with a teal glow shadow and a 1° rotation — feels like
 *    a real screenshot instead of a flat asset dropped on white
 * 4. The headline breaks deliberately at the comma so the gradient word
 *    sits on its own visual beat
 */
export default function Hero() {
  ...
  return (
    <section className="relative flex items-center pt-16 overflow-hidden bg-white dark:bg-slate-950 min-h-screen">
      ...
      {/* Dot grid background — fades toward the center via a radial mask */}
      <div
      ...
      {/* Accent blobs — subtler than before, more of a tint than a shape */}
      ...
      <div className="relative w-full max-w-6xl mx-auto px-4 sm:px-6 py-16 sm:py-20 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="visible" className="flex flex-col items-start">
          ...
          {/*
            H1 — three visual beats:
              "Del caos al flujo,"  → slate-900 / white
              "en un solo lugar."   → gradient teal-to-cyan via .text-gradient-teal
            Breaking at the comma makes the gradient word land on its own line
            at most viewport widths, giving it more visual weight.
          */}
          <h1 className="font-heading font-bold text-slate-900 dark:text-white text-fluid-hero mb-5 max-w-xl">
            Del caos al flujo,{" "}
            <span className="text-gradient-teal">en un solo lugar.</span>
          </h1>
        ...
        >
          {/*
            Floating mockup treatment:
            - Outer div creates a teal ambient glow via blur
            - Inner div applies a 1° clockwise tilt — breaks the "dropped on
              a white background" feeling and adds a sense of depth/perspective
          */}
        ...
        >
          {MOBILE_PILLS.map(({ Icon, label, bg, text }) => (
            <div key={label} className={`${bg} border rounded-xl p-3 sm:p-4 flex flex-col items-center gap-2 text-center`}>
